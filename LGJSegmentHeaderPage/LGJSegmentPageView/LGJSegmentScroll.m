...
- (void)setContentOffset:(CGPoint)contentOffset animated:(BOOL)animated {
    ...
    if (!animated) {
        if ([self.delegate respondsToSelector:@selector(animationEndIndex:)]) {
            [self.delegate animationEndIndex:currentIndex];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if ([self.delegate respondsToSelector:@selector(scrollOffsetScale:)]) {
        [self.delegate scrollOffsetScale:scale];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    //滑动结束
    NSInteger currentIndex = self.contentOffset.x/self.frame.size.width;
    if ([self.delegate respondsToSelector:@selector(scrollEndIndex:)]) {
        [self.delegate scrollEndIndex:currentIndex];
