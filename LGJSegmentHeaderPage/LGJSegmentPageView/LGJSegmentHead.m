...
@interface LGJSegmentHead()

//MARK- 其他设置
/**
 *  LGJSegmentHeadStyle
 */
@property(nonatomic, assign) LGJSegmentHeadStyle headStyle;
/**
 *  LGJSegmentLayoutStyle
 */
@property (nonatomic, assign) LGJSegmentLayoutStyle layoutStyle;

@end


@implementation LGJSegmentHead
{
...
- (instancetype)initWithFrame:(CGRect)frame titles:(NSArray *)titles headStyle:(LGJSegmentHeadStyle)style {
    return [self initWithFrame:frame titles:titles headStyle:style layoutStyle:LGJSegmentLayoutDefault];
}

- (instancetype)initWithFrame:(CGRect)frame titles:(NSArray *)titles headStyle:(LGJSegmentHeadStyle)style layoutStyle:(LGJSegmentLayoutStyle)layout {
...
- (void)defaultAndCreateView {
    ...
    //判断总宽度
    if (sum_width > SCROLL_WIDTH && _layoutStyle== LGJSegmentLayoutCenter) {
        _layoutStyle = LGJSegmentLayoutLeft;
    }
...
- (void)titlesWidth {
    ...
    for (NSString *title in titlesArray) {
        if (_layoutStyle != LGJSegmentLayoutDefault) {
...
- (void)scrollViewSubviews:(UIScrollView*)scroll {
    ...
    if (_layoutStyle == LGJSegmentLayoutCenter) {
        //计算布局的起点
